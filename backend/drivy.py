...
def commission(price,nb_of_day): #Given the price and the duration of the rental, gives the part that goes to each entity
    repartition=[]
    repartition.append(action("driver",price))
    commission_price=price*0.3 #Total price of the commission
    repartition.append(action("owner",price-commission_price)) #Part that goes to the owner
...
with open("data/input.json","r") as file:
    ...
    for rental in rentals: #For each rental
        ...
        #Writing of the JSON
        dico["id"]=rental["id"]
        dico["actions"]=commission(rent_price,period)
        prices.append(dico)
